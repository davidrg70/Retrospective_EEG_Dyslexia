...
switch eventformat
    ...
    case 'edf'
        ...
        elseif issubfield(hdr, 'orig.annotation') && ~isempty(hdr.orig.annotation) % EDF itself does not contain events, but EDF+ does define an annotation channel
            ...
            %ADDED by NF to comply with the offset in TAL % ADDED BY NF 2018 08 01
            tok = tokenize(tal{1}, char(20));% ADDED BY NF 2018 08 01
            offset=str2double(tok{1});% ADDED BY NF 2018 08 01
            event = [];
            for i=2:length(tal)
                ...
                if any(tok{1}==21)
                    ...
                    time     = str2double(dum{1})-offset; % ADDED BY NF 2018 08 01
                    duration = str2double(dum{2});
                else
                    % only the time is specified
                    time     = str2double(tok{1})-offset; % ADDED BY NF 2018 08 01
                    duration = [];
                end
