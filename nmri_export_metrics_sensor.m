  ...
  for si=1:length(all_layouts)
   ...
   for li=1:length(all_layouts{si}.label)
    ...
    if length(idx)==1
     ...
     fprintf(['Could not find a unique mapping for channel (' all_layouts{si}.label{li} ') in the current subject data and the master layout (1st subject). Skipping this channel.\n'])
    end
   end
  ...
  for global_i=1:length(opt.global)
   ...
   for dim_i=1:length(opt.dim_reduction)
    ...
    for scale_i=1:length(opt.scale)
     ...
     filebase=fullfile(outputdir,[ opt.metrics{m} '_' freq_txt{f} '_' global_txt '_' scale_txt dim_txt interp_txt '_sensor_N' num2str(length(all_subjects))]);
     if opt.save_mat
      fprintf('Saving as .mat...')
      if opt.interpolate
       active_hdm_class='interp_sensor';
      else
       active_hdm_class='sensor';
      end
