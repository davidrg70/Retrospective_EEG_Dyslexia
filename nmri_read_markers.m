...
if isfield(params,'readEvents')
    ...
    for evC=1:length(allEvts)
        ...
        if isfield(params.readEvents.(allEvts{evC}),'regexp')
            regexpPattern=params.readEvents.(allEvts{evC}).regexp;
            regexpPattern=strrep(regexpPattern,'<subject_id>',subject.id);
            regexpPattern=strrep(regexpPattern,'<exam_id>',subject.exam_id);
            regexpPattern=strrep(regexpPattern,'<raw_dataset>',subject.raw_dataset); % dataset, e.g. of EDF reader
            regexpPattern=strrep(regexpPattern,'<raw_datadir>',fileparts(subject.raw_dataset)); % path of raw dataset
            regexpPattern=strrep(regexpPattern,'<analysis_dir>',subject.analysis_dir);
        else
            regexpPattern={};
        end
        ...
        for iP=1:length(pattern)
            ...
            for iF=1:length(eventFileThis)
                if ~isempty(regexpPattern) && ~all(cellfun(@isempty,regexp(eventFileThis(iF).name,regexpPattern,'once')))
