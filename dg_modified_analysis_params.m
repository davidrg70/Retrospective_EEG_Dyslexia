...
% Trial definiation
params.trial_length = 2; % in seconds
...
params.freqs = [2 6 10 16 25 ]; % in Hz, center frequency of interest
% params.freqs = [2 6 10 16 25 40 ]; % in Hz, center frequency of interest
params.tapsmofrq = [2 2 2 4 4 ]; % plus minus the frequency smoothing
% params.tapsmofrq = [2 2 2 4 4 8 ]; % plus minus the frequency smoothing

params.freqsNames = {'Delta' 'Theta' 'Alpha' 'Beta1' 'Beta2'};
...
% use .Path for the search paths that are matched via "dir". 
% Note: this function uses the plain * as wildcard

% use .regexp to Matlab regexp match the resulting file names. 
% Note: this function uses full Matlab/Posix wildcard system, e.g.
% .*,^,$,..
...
% to read EGI/MFF
params.readEvents.MFF.Class='Auto'; % Auto assign to spikes or info
params.readEvents.MFF.Path = {'<raw_dataset>','<analysis_dir>/spike_markings'}; % check the dataset itself + spike_markings
params.readEvents.MFF.regexp = {'Event.*\.xml$','<subject_id>.*\.xml$'}; % parse xml Event files only
params.readEvents.MFF.EventsFunction='nmri_read_event_mff'; % 
params.readEvents.MFF.EventsTimebase = 1; % For EGI EEG already seconds
...
params.con_method={'wpli_debiased','coh'}; % deafulting to coherency only (will include imCoh)
